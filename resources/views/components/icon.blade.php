...
@if($name === 'dnsc')
    <img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+ip1sAAAAASUVORK5CYII=" alt="Icon" class="{{ $class }}" />
@elseif($name === 'star')
    <svg viewBox="0 0 24 24" class="{{ $class }}" fill="currentColor" stroke="none">{!! $icons['star'] !!}</svg>
@elseif(isset($icons[$name]))
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="{{ $class }}">
        {!! $icons[$name] !!}
    </svg>
@endif
