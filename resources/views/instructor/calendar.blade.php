...
@section('title', 'Calendar - Instructor Console')

@section('content')

<x-page-header title="Activity Calendar" subtitle="Schedule of classes and official activities">
</x-page-header>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
        ...
        <x-card class="p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('instructor.calendar', ['month' => $prevMonth->month, 'year' => $prevMonth->year]) }}" class="p-2 rounded hover:bg-slate-50 text-slate-400 inline-block shrink-0" title="Previous Month">
                        <x-icon name="chevron" class="w-4 h-4 rotate-90" />
                    </a>
                    <a href="{{ route('instructor.calendar', ['month' => $nextMonth->month, 'year' => $nextMonth->year]) }}" class="p-2 rounded hover:bg-slate-50 text-slate-400 inline-block shrink-0" title="Next Month">
                        <x-icon name="chevron" class="w-4 h-4 -rotate-90" />
                    </a>
                </div>
            </div>
                ...
                @for($d = 1; $d <= $selectedMonth->daysInMonth; $d++)
                    ...
                    <div class="border {{ $isToday ? 'border-indigo-400 ring-2 ring-indigo-50/50 bg-indigo-50/10' : 'border-slate-200' }} rounded-xl p-3 min-h-[100px] bg-white hover:border-indigo-300 hover:shadow-sm transition duration-205 cursor-pointer">
                        <div class="flex justify-between items-center mb-1">
                            <span class="text-sm font-bold {{ $isToday ? 'text-indigo-650 bg-indigo-100/70 w-6 h-6 rounded-full flex items-center justify-center -mt-1 -ml-1 text-xs shrink-0 font-extrabold' : 'text-slate-750' }}">{{ $d }}</span>
                        </div>
    ...
    <div class="lg:col-span-1">
        <x-card title="Upcoming Activities" subtitle="Your schedule for the month">
            <ul class="divide-y divide-slate-100 mt-2">
                ...
                <li class="px-5 py-4 flex items-center gap-4 hover:bg-slate-50 cursor-pointer transition">
                    <div class="w-1.5 self-stretch rounded-full {{ $a->color ?? 'bg-indigo-500' }}"></div>
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="text-xs text-slate-500 mt-1 flex items-center gap-3 flex-wrap font-medium">
                            ...
                            <span class="inline-flex items-center gap-1"><x-icon name="clock" class="w-3.5 h-3.5" />08:00 AM</span>
                        </div>
                    </div>
                </li>
                @empty
                <li class="py-8 text-center text-slate-400 text-sm">No upcoming activities.</li>
                @endforelse
            </ul>
        </x-card>
    </div>
